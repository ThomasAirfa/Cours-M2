 ...
 *
 * The method {@link #clear(Color)} can be used to set a background color. The
 * method {@link #render(Consumer)} can be used to draw something one screen.
 *
 * The mouse events can be retrieved using
 * {@link #waitForMouseEvents(MouseCallback)}.
 *
 * All these methods are thread safe. However, the consumer provided to render
 * is run in a different thread than the own calling it. This can create data
 * races if the consumer uses non-final values.
 *
 */
